...
- **Form**
  Базовый класс для всех форм приложения. Реализует типовую обработку событий `input` (эмитит изменения полей) и `submit` (эмитит событие отправки формы). Дочерние классы (`Order`, `Contacts` и др.) расширяют его для специфики своих форм.
...
- **Card**
  Отображает карточку товара, хранит только id товара (а не весь объект), обрабатывает клики по карточке и кнопке добавления/удаления из корзины. Для получения остальных данных использует AppState.

- **Basket**
  Отображает содержимое корзины, общую сумму, кнопку оформления заказа.

- **BasketItem**
  Отображает отдельный товар в корзине, хранит только id товара, обрабатывает удаление товара. Для получения остальных данных использует AppState.
